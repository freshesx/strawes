<template>
  <div class="mw-table-column">
    <div class="mw-table-column-control" @click="onOpenModal">
        ...
        <mn-section>
          <mn-card>
            ...
            <mn-card-item v-for="(column, key) in computedColumns" :key="key">
              <mn-card-prefix>
                <mw-table-check :checked="isOneSelected(column)" @click="onOneSelected(column)"></mw-table-check>
              </mn-card-prefix>
              <mn-card-body>
                {{ column.title }}
              </mn-card-body>
